<!--
  Chapter Reader - Auto-hide navbar, reading history tracking, chapter navigation
-->
<script lang="ts">
...
>
	...
	>
		<div class="flex items-center justify-between px-3 py-2 bg-gradient-to-b from-black/95 to-transparent backdrop-blur-sm">
			<!-- Left: Back button -->
			<button
				onclick={goBack}
				class="flex items-center gap-2 px-3 py-1.5 bg-white/10 hover:bg-white/20 rounded-lg text-sm transition-colors"
			>
			...
			<div class="flex items-center gap-1 sm:gap-2">
				...
				<div class="relative">
					<button
						...
						class="flex items-center gap-1 px-3 py-1.5 bg-white/10 hover:bg-white/20 rounded-lg text-sm transition-colors max-w-[200px]"
					>
					...
					<!-- Chapter Dropdown -->
					{#if showChapterList}
						<div class="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-64 max-h-80 overflow-y-auto bg-zinc-900/95 backdrop-blur-md border border-zinc-700 rounded-lg shadow-xl">
							{#each chapters as chapter}
								<button
									onclick={() => goToChapter(chapter.id)}
									class="w-full px-4 py-2 text-left text-sm hover:bg-[var(--color-primary)]/30 transition-colors {chapter.id === chapterId ? 'bg-[var(--color-primary)]/20 text-[var(--color-primary)]' : 'text-zinc-300'}"
								>
									{chapter.title}
								</button>
							{/each}
			...
			<div class="flex gap-2">
				<span class="text-xs text-zinc-500 hidden sm:flex items-center">
					{currentPage + 1}/{pages.length}
				</span>
				<button
					onclick={toggleMode}
					class="px-2 py-1.5 bg-white/10 hover:bg-white/20 rounded-lg text-xs transition-colors"
				>
